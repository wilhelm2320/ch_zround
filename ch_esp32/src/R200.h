...
class R200 {
  ...
  private:
    ...
    bool dataIsValid();
    unsigned int receiveData(unsigned long timeOut = 500);
  ...
  public:
    ...
    unsigned int car_id;
    uint8_t uid[12] = {0};
    int GetReceiverDemodulatorParameters_Mixer_G();
    int GetReceiverDemodulatorParameters_IF_G();
    int GetReceiverDemodulatorParameters_threshold();
    int SetModuleIdleSleepTime (uint8_t);
    int GetWorkArea();
    int SetWorkArea(uint8_t);
    int GetSinglePollInstruction();
    int GetMultiPollInstruction(uint8_t * ui_Buffer);
    int SetQueryParameters(uint16_t );    
    int SetBuzzerOnOff(byte );    
    uint16_t GetQueryParameters();
    int SetReceiverDemodulatorParameters_Mixer_G(byte,byte,uint16_t);
    int SetTransmitPower(uint16_t value);
    uint16_t AcquireTransmitPower();
